...
import { generatePPT } from "./api/useChat";

interface AssistantProps {
  setMarkdown: (text: string) => void;
}

export const Assistant: React.FC<AssistantProps> = ({ setMarkdown }) => {
  const runtime = useLocalRuntime({
    run: async ({ messages, abortSignal }) => {
      ...
      const a = await generatePPT(messageMap);
      if (a.includes('# End')) {
        setMarkdown(a);
      }
      return {
  ...
  return (
    <div className="flex flex-col h-screen max-w-[30rem]">
    <div className="flex items-center gap-3 border-b px-6 py-4">
        <div className="flex items-center justify-center">
          <Atom className="h-6 w-6 text-primary animate-pulse" aria-hidden="true" />
        </div>
        <div>
          <h1 className="text-lg font-semibold text-white">Genesis</h1>
          <p className="text-sm text-muted-foreground">Because you think AI is better at this stuff</p>
        </div>
      </div>
